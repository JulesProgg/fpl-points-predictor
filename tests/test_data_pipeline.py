...
def test_select_useful_handles_missing_useful_columns():
    """select useful should work even if some columns don't exist"""
    df = pd.DataFrame(
        {
            "name": ["Player A"],
            "team": ["Team X"],
...
def test_run_pipeline_reads_three_csvs_and_writes_output(tmp_path, monkeypatch):
    ...
    # keep the real pandas.read_csv function aside in _real_read_csv
    _real_read_csv = pd.read_csv

    def fake_read_csv(path, *args, **kwargs):
        path_str = str(path)
        # Allow reading the final output csv with the true pandas function
        if "players_all_seasons.csv" in path_str:
